...
interface JoinQueueDialogProps {
  ...
  onJoin: (players: Array<{ name: string; skillLevel: string }>, groupSize: number) => void
}

export function JoinQueueDialog({ open, onOpenChange, onJoin }: JoinQueueDialogProps) {
  const [groupSize, setGroupSize] = useState("1")
  const [players, setPlayers] = useState([{ name: "", skillLevel: "intermediate" }])

  const handleGroupSizeChange = (value: string) => {
    setGroupSize(value)
    const size = Number.parseInt(value)
    if (size > players.length) {
      setPlayers([...players, ...Array(size - players.length).fill({ name: "", skillLevel: "intermediate" })])
    } else {
      setPlayers(players.slice(0, size))
    }
  }

  const handlePlayerChange = (index: number, field: "name" | "skillLevel", value: string) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card border-border max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Enter your details to join the queue for this event</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {Number.parseInt(groupSize) > 1 && (
            <Alert>
              ...
              <AlertDescription>
                You're joining as a group of {groupSize}. All players will be assigned to the same court together.
              </AlertDescription>
            </Alert>
          )}
            ...
            {players.map((player, index) => (
              <div key={index} className="p-4 border border-border rounded-lg space-y-3">
                <h4 className="font-medium text-foreground">
                  Player {index + 1}
                  {index === 0 && Number.parseInt(groupSize) > 1 && " (Group Leader)"}
                </h4>
                <div className="grid md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handlePlayerChange(index, "skillLevel", value)}
                    >
