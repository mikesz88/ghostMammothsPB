...
| Test UI | `components/admin/events/test-controls.tsx` | Test-event-only card; calls `useTestControls` from `lib/hooks/admin/`. |
| Realtime + actions | `lib/hooks/admin/use-admin-event-detail-realtime.ts`, `use-admin-event-detail-actions.ts` | Supabase channels for `queue_entries` / `court_assignments`; assign next, clear queue, remove entry, end game (toasts + server actions). If leaves don’t appear for other viewers, see [`supabase-realtime-queue-rls.md`](supabase-realtime-queue-rls.md). |
| Test helpers hook | `lib/hooks/admin/use-test-controls.ts` | State + handlers for dummy queue / rotation / team size / court count (test-helpers actions). |
...
* **Hooks under `lib/hooks/`:** Matches Phase 2 (`use-event-detail-*` under `lib/hooks/event-detail/`); admin/event dashboard hooks under `lib/hooks/admin/`. Keeps `components/admin/events/*` mostly presentational and avoids architecture-audit “persistence in components” noise for Supabase browser usage.
