...
import { Trophy, Plus, Calendar, MapPin, Settings, Trash2, Edit, ExternalLink } from "lucide-react"
...
export default function AdminPage() {
  ...
  const handleEndEvent = (eventId: string) => {
    const updatedEvents = events.map((e) => (e.id === eventId ? { ...e, status: "ended" as const } : e))
    setEvents(updatedEvents)
  }

  const handleDeleteEvent = (eventId: string) => {
    if (
      confirm("Are you sure you want to delete this event? This will remove all queue entries and court assignments.")
    ) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-foreground mb-2">Event Management</h1>
            <p className="text-muted-foreground">Create and manage pickleball events</p>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          ...
          <Card className="border-border bg-card">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Total Courts</p>
          ...
          <Card className="border-border bg-card">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Ended Events</p>
                  <p className="text-3xl font-bold text-foreground">{endedEvents.length}</p>
                </div>
        ...
        {/* Active Events */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-foreground mb-4">Active Events</h2>
          {activeEvents.length === 0 ? (
            <Card className="border-border bg-card">
              <CardContent className="p-12 text-center">
                <p className="text-muted-foreground mb-4">No active events</p>
                <Button onClick={() => setShowCreateDialog(true)}>Create Your First Event</Button>
              </CardContent>
            </Card>
          ) : (
        ...
        {endedEvents.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold text-foreground mb-4">Ended Events</h2>
            <div className="grid md:grid-cols-2 gap-6">
              {endedEvents.map((event) => (
                <Card key={event.id} className="border-border bg-card opacity-60">
                  <CardHeader>
                    <div className="flex items-start justify-between mb-2">
      ...
      <CreateEventDialog open={showCreateDialog} onOpenChange={setShowCreateDialog} onCreate={handleCreateEvent} />
      {editingEvent && (
        <EditEventDialog
          open={!!editingEvent}
